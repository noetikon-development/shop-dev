...
| `CouponRedemption` | Step 14. One row per successful coupon application (`orderId` unique); authoritative for global + per-customer usage limits (checkout locks the parent `Coupon` row, counts rows whose order isn't `CANCELLED`). Excluded from `seed.sql`. Step 18 adds a `createdAt` index for coupon analytics |
| `User` | application record only — **no password**; `supabaseUserId` unique link to `auth.users`; `role` is a coarse mirror of the RBAC tables. Step 18 adds a `createdAt` index for new-customer analytics |
...
| `OrderItem` / `OrderEvent` | present; excluded from `seed.sql`. Step 18 adds `OrderItem(productId)` index for best-seller / product-performance group-bys |
