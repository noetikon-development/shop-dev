...
async function runCheckoutCoreWithCoupon(
  ...
  // 9F-15B: THIRD_PARTY COD orders are auto-confirmed to PROCESSING at checkout.
  const autoConfirm = soSeller.type === "THIRD_PARTY";
  const order = await tx.order.create({ data: { orderNumber, userId: args.userId, cartId: args.cartId, email: args.userEmail, phone: args.shipAddr.phone, status: autoConfirm ? "PROCESSING" : "PENDING_PAYMENT", paymentMethod: "NONE", paymentStatus: "PENDING", subtotal, shippingFee, discountTotal, grandTotal, couponId: args.coupon.id, couponCode: args.coupon.code, discountType: args.coupon.type, discountValue: args.coupon.value, shippingMethodId: args.method.id, shippingMethod: args.method.code, shippingMethodCode: args.method.code, shippingMethodName: args.method.name, addressId: args.shipAddr.id, billingAddressId: args.shipAddr.id, shippingAddress: "{}" }, select: { id: true } });
