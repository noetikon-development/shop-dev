...
function staticTests() {
  ...
  ok("trigger · cancelOrderAction captures affected SellerOrder ids before the update (not a blind notify-all)", /toCancel = await tx\.sellerOrder\.findMany/.test(orderActions) && /cancelledSellerOrderIds = toCancel\.map/.test(orderActions));
  ...
  ok("trigger · markReceivedAction still fires the unchanged customer return_received email, then loops sellers", /scheduleEmail\(\(\) => sendReturnReceived\(ret\.id\)\);[\s\S]{0,700}getReturnAffectedSellerIds/.test(returnsActions));
