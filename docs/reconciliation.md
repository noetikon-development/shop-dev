...
marketplace) once a day, plus an hourly watchdog that catches the one failure
mode the job itself can't detect: a run that silently died mid-execution.
...
RUNNING ──┬─→ PASS     (both checks clean)
          ...
          └─→ ERROR    (the job itself threw before producing a result —
                         ...
                         OR the hourly watchdog found the row still RUNNING
...
- **Schedule:** hourly, `0 * * * *` → `GET /api/cron/reconciliation-watchdog` (`vercel.json`), independent of the daily job's own schedule.
...
The first two are naturally "once per day" events (the job runs once a day). The stale-run alert is keyed to the specific `ReconciliationRun.id` instead, because the watchdog itself runs hourly and must not re-alert every hour about the same already-handled stale run — while still guaranteeing a *different* stale run (a different id, possibly the very next day's run) is never suppressed by an unrelated key collision.
